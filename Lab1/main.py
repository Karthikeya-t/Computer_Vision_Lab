import cv2
import numpy as np
def read_and_display_image(img_path):
    #read image
    #img = cv2.imread(img) #we can use this line if we are read the image (if only path is passed)
    # Display the image
...
if __name__ == '__main__':
    ...
    read_video(mode, r'videos/video.mp4' if mode == "video" else None)
    # img_path = cv2.imread(r'input_image.jpg')  # Read an image using the relative path
    # scale_down_up_image(img_path)
    #check_colour_space_image(img_path)
    #read_and_display_image(img_path)
    # message="""
